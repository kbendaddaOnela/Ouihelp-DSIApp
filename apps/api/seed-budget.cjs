 ...
 * Usage : node seed-budget.js
 * (exécuter depuis apps/api)
 */
const fs = require('fs')
...
const CD = 'Colisee Domicile'
const ONS = 'ONELA Services'
...
async function seed() {
  ...
  for (const item of ITEMS) {
    const id = randomUUID()
    const status = computeStatus(item.contractEnd)
    try {
      await pool.execute(
        `INSERT INTO budget_items
          (id, name, vendor, category, quantity, unit_cost, amount, currency,
           billing_cycle, contract_start, contract_end, auto_renewal,
           renewal_alert_days, status, billing_entity, notes, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, 'EUR', ?, ?, ?, 0, 60, ?, ?, ?, ?, ?)`,
        [
          id, item.name, item.vendor, item.category, item.quantity,
          item.unitCost, item.amount, item.billingCycle,
          item.contractStart, item.contractEnd, status,
          item.billingEntity, item.notes, now, now,
        ]
      )
